...
graph TD
    ...
    subgraph Layered
        C1[分层架构] --> C2[Controller层：接收请求]
        C1 --> C3[Service层：业务逻辑]
        C1 --> C4[Repository层：数据访问]
        C1 --> C5[Model层：数据模型]
    end
...
```mermaid
graph TD
    X1[XSS防御] --> X1a[输入过滤、输出编码、CSP头部]
    X2[CSRF防御] --> X2a[Token验证、SameSite Cookie]
    X3[SQL注入] --> X3a[参数化查询、ORM安全使用]
    X4[认证安全] --> X4a[JWT合理过期、OAuth2、2FA]
    X5[速率限制] --> X5a[API限流、DoS防御]
