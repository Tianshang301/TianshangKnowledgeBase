...
| 并联端接 | R_p(到 Vcc/GND)| 信号质量好 | 功耗大 |
| AC 端接 | R+C 串联到 GND | 无直流功耗 | 占用空间 |
| 戴维南端接 | R1到 Vcc+R2到 GND | 双向匹配 | 功耗大 |
| 差分端接 | Rt 跨接差分对 | 匹配精度高 | 仅差分信号 |
...
| 1-100MHz | 0.1-1uF | 0805/0603 | IC 附近 | 1/IC |
| > 100MHz | 10-100nF | 0402/0201 | IC 引脚旁 | 多颗 |
...
```python
# Python 使用 PyEDA 或 scikit-rf 进行 S 参数分析
import numpy as np
...
3. IPC-2221 PCB 设计通用标准
4. Altium Designer 高速 PCB 设计指南
