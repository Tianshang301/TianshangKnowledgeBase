...
BibTeX 是 LaTeX 的参考文献管理工具，由 Oren Patashnik 于 1985 年开发。它通过单独的 .bib 文件存储文献数据，配合 .bst 格式文件自动生成格式化的参考文献列表。BibLaTeX 是其后续方案，提供更多功能和灵活性。

### BibTeX 工作流程

```mermaid
flowchart LR
    TEX[.tex 文件<br/>正文 + cite] --> AUX[.aux 文件<br/>引用信息]
    BIB[.bib 文件<br/>文献数据库] --> BBL[.bbl 文件<br/>格式化文献]
    BST[.bst 文件<br/>格式定义] --> BBL
    AUX --> BIBTEX[bibtex 编译]
    BIBTEX --> BBL
    BBL --> PDF[最终 PDF]
```
...
\bibliographystyle{plain}  % 指定 .bst 格式
\begin{document}
...
| alpha | [Knu84] 作者年份缩写 | 按标签字母顺序排列 |
| abbrv | [1] 数字编号 | 名字、月份等缩写 |
| apalike | (Knuth, 1984) | 作者-出版年制 |
...
| 中文文献 | 使用 `ctex` 包，BibTeX 配合 `gbt7714` 格式 |
