---
aliases: [KnowledgeGraphBasics, KG基础]
...
| 层次 | 说明 | 示例 |
|------|------|------|
| 模式层 | 定义类、属性、关系的结构 | Person类有name属性 |
| 数据层 | 具体的实体实例 | 爱因斯坦是Person的实例 |
| 推理层 | 基于规则推导新知识 | 若A是B的子类，B的实例也是A的实例 |
...
RDF（Resource Description Framework）是W3C制定的知识表示标准框架。

**RDF序列化格式**：

| 格式 | 全称 | 特点 |
|------|------|------|
| RDF/XML | XML语法表示 | 标准规范，可读性差 |
| Turtle | Terse RDF Triple Language | 简洁易读，推荐使用 |
| N-Triples | 每行一个三元组 | 最简格式，适合大规模 |
| JSON-LD | JSON for Linked Data | Web友好，JSON集成 |
...
| CONSTRUCT | `CONSTRUCT { ... }` | RDF图 |
| ASK | `ASK` | 布尔值 |
| DESCRIBE | `DESCRIBE ?x` | 描述资源的RDF图 |
...
| 关系处理 | 天然支持，高效遍历 | 需要JOIN操作 |
| 模式灵活性 | Schema-less或灵活模式 | 固定模式 |
...
| ArangoDB | 多模型数据库 | AQL | 文档+图+KV三合一 |
