...
| 知识发现 | 需手动遍历 | 自动发现关联 |
| 维护成本 | 低 | 低（自动） |
| 典型工具 | Wiki、HTML a 标签 | Roam Research、Obsidian、Logseq |
...
```mermaid
graph TD
    A[笔记 A<br/>关于图论] -->|链接| B[笔记 B<br/>关于神经网络]
    A -->|链接| C[笔记 C<br/>关于矩阵运算]
    B -->|链接| D[笔记 D<br/>关于深度学习]
    C -->|链接| D
    D -->|链接| E[笔记 E<br/>关于计算机视觉]
    A -->|反向链接| F[笔记 F<br/>引用图论]
```
...
|------|---------|---------|------------|---------|
| Obsidian | Wiki 链接 | 力导向图 | 侧边栏 | 插件生态丰富 |
| Roam Research | Block-level | 无 | 右侧面板 | 块级引用、大纲编辑 |
| Logseq | 大纲链接 | 有 | 底部面板 | 开源、Markdown 优先 |
| Notion | 页面链接 | 无 | 反向链接列表 | 数据库联动 |
| TiddlyWiki | Transclusion | 无 | 链接列表 | 单 HTML 便携 |
...
[[05_ComputerScience/DigitalNotes/DigitalNoteTaking|DigitalNoteTaking]], [[00_KnowledgeFramework/Methodology/SecondBrain|SecondBrain]], [[02_NaturalSciences/Logic/MathematicalLogic/GraphTheory|GraphTheory]], [[INDEX|知识库索引]]
