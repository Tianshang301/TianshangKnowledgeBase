...
| 编码 | 原理 | 压缩率 | 解码速度 | 适用场景 |
|:----:|------|:------:|:--------:|---------|
| 可变字节编码（VByte）| 每个字节 7 位数据 + 1 位标识 | 中 | 快 | 通用 |
| Gamma 编码 | 一元码 + 二进制，小整数高效 | 高 | 中 | 小文档间距 |
| Rice 编码 | 参数化的 Golomb 编码 | 高 | 中 | 已知分布 |
| PForDelta | 打包 FOR 差分编码 | 极高 | 快 | 大规模搜索索引 |
...
**概念图评价**：Novak 提出依据层级、交叉连接和示例数量量化评估。
### 3.2 思维导图（Mind Map）
...
- FM-index: Ferragina, P., & Manzini, G. (2005). Indexing Compressed Text. *JACM*, 52(4), 552-581.
