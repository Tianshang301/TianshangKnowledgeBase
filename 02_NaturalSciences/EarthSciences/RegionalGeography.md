...
```mermaid
mindmap
  root((Regional Geography))
    区域理论 Region Theory
      均质区 Homogeneous Region
      功能区 Functional Region
      感知区 Perceptual Region
      区域系统 Regional System
      区域尺度 Scale Hierarchy
    区域自然地理
      气候 Climate
      地形地貌 Landform
      水文 Hydrology
...
```mermaid
flowchart LR
    A[全球热点区域] --> B[亚太 Asia-Pacific]
    A --> C[欧洲 Europe]
    A --> D[中东 Middle East]
    A --> E[非洲 Africa]
    A --> F[北极 Arctic]

    B --> B1[中美竞争 US-China Rivalry]
    B --> B2[南海 South China Sea]
    C --> C1[俄乌冲突 Ukraine War]
    C --> C2[能源转型 Energy Transition]
    D --> D1[以巴冲突 Israel-Palestine]
    D --> D2[石油地缘 Oil Geopolitics]
    E --> E1[发展治理 Development Gap]
    F --> F1[航道争夺 Shipping Route]
    F --> F2[资源开发 Resource Extraction]
```
