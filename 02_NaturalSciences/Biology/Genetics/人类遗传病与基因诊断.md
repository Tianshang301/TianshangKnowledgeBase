...
| 类型 | 机制 | 疾病示例 |
|------|------|---------|
| 缺失（Deletion） | 染色体片段丢失 | Cri-du-chat综合征（5p-） |
| 重复（Duplication） | 片段重复 | Charcot-Marie-Tooth病 |
...
### 3.3 X连锁隐性遗传 (X-linked Recessive, XR)
...
| 疾病 | 突变位点 | 主要症状 |
|------|---------|---------|
| Leber遗传性视神经病变（LHON） | MT-ND4, MT-ND6 | 急性视力丧失 |
| MELAS综合征 | MT-TL1 | 线粒体脑肌病 |
...
通过G显带（G-banding）分析染色体数目和结构。

### 5.2 FISH (Fluorescence In Situ Hybridization)

使用荧光标记探针检测特定DNA序列，分辨率达100kb。
...
| 技术 | 原理 | 应用 |
|------|------|------|
| 常规PCR | 扩增特定DNA片段 | 突变检测 |
| 实时定量PCR（qPCR） | 荧光定量 | 基因表达 |
| 数字PCR（dPCR） | 绝对定量 | 低频突变检测 |
| 多重PCR（Multiplex PCR） | 多对引物 | 多基因检测 |
...
CRISPR/Cas9系统由sgRNA引导Cas9蛋白切割靶DNA：

$$
\text{sgRNA} + \text{Cas9} \xrightarrow{\text{PAM序列识别}} \text{双链断裂 (DSB)}
$$
...
人类遗传病涵盖染色体、单基因和多基因等多种类型。基因诊断技术的快速发展使更多疾病得以准确诊断。基因治疗和CRISPR编辑为遗传病的治疗提供了新希望，但同时也带来伦理挑战。
