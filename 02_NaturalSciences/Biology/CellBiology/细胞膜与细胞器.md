...
由 Singer 和 Nicolson 在1972年提出，核心内容包括：
...
| 类型 | 能量来源 | 示例 |
|------|---------|------|
| 初级主动运输（Primary） | 直接水解 ATP | Na⁺/K⁺ ATP 酶 |
| 次级主动运输（Secondary） | 离子梯度 | Na⁺/葡萄糖同向转运 |

Na⁺/K⁺ ATP 酶每水解1分子 ATP 运输3个 Na⁺出细胞，2个 K⁺入细胞。
...
| 药物解毒（Detoxification） | 细胞色素 P450系统 |
...
$$
\text{蛋白} \xrightarrow{\text{顺面}} \text{添加 N-连接寡糖} \xrightarrow{\text{中间}} \text{修饰} \xrightarrow{\text{反面}} \text{添加 O-连接糖链}
$$

## 7. 溶酶体 (Lysosome)

溶酶体含有约60种酸性水解酶（Acid Hydrolase），最适 pH 约5.0。

溶酶体酶在 TGN 被甘露糖-6-磷酸（M6P）标记分选：
...
|------|------|---------|
| 外膜（OMM） | 通透性高 | 物质交换 |
| 膜间隙（IMS） | 含 Cyt c | 凋亡信号 |
| 内膜（IMM） | 折叠成嵴 | 电子传递链 |
| 基质（Matrix） | 含 DNA 和核糖体 | TCA 循环 |
...
质子驱动力（Proton Motive Force, PMF）驱动 ATP 合酶合成 ATP：

$$
ADP + P_i \xrightarrow{\text{ATP 合酶}} ATP
$$
