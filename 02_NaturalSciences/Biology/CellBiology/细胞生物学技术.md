...
荧光显微镜利用荧光染料（Fluorophore）或荧光蛋白（如GFP）标记目标分子。
...
突破衍射极限的技术包括STED、PALM和STORM，分辨率可达20-50nm。
...
| 共定位分析（Colocalization） | 测量两种荧光重叠程度 | Pearson相关系数 |
| 追踪分析（Tracking） | 追踪细胞/粒子运动 | TrackMate |
| 三维重建（3D Reconstruction） | Z-stack图像合成3D | Imaris、Amira |
...
基于细胞密度差异分离，使用Ficoll或Percoll梯度介质：
