...
- $p(\theta)$：先验分布（Prior）
- $p(y \mid \theta)$：似然（Likelihood）
- $p(\theta \mid y)$：后验分布（Posterior）
- $p(y)$：边际似然（Evidence）
...
| Normal$(\mu, \sigma^2)$<br/>$\sigma^2$ 已知 | Normal$(\mu_0, \sigma_0^2)$ | Normal$(\frac{\mu_0/\sigma_0^2 + n\bar{y}/\sigma^2}{1/\sigma_0^2 + n/\sigma^2}, \frac{1}{1/\sigma_0^2 + n/\sigma^2})$ |
...
- 独立的概率编程语言
- 使用 HMC/NUTS 高效采样
- Python 接口：PyStan（CmdStanPy）
- 适合复杂模型

## 六、应用

- **A/B 测试**：贝叶斯方法提供 $P(\text{A 优于 B})$ 的直接概率
- **贝叶斯分层模型**：处理分组数据（如不同学校的考试成绩）
- **贝叶斯优化**：超参数调优（Gaussian Process + acquisition function）
- **贝叶斯时间序列**：结构时间序列模型
