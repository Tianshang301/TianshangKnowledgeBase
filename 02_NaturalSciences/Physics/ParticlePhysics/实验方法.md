...
- **CMS** (Compact Muon Solenoid)：紧凑型缪子螺线管探测器，与 ATLAS 互验
- **ALICE** (A Large Ion Collider Experiment)：研究夸克-胶子等离子体
- **LHCb**：研究底夸克物理和 CP 破坏
- **BESIII** (北京谱仪 III)：在 BEPCII 上的陶-粲物理实验
...
LHC 每秒产生40百万次束团交叉 (40 MHz bunch crossing)，但只有约1 kHz 的事件值得存储。触发系统分多级筛选：
...
DAQ 系统将触发选中的事件数据组装并通过高速网络传输到存储系统。LHC 实验每年产生约 50 PB 的数据。
...
| 中微子物理 | JUNO, DUNE, Hyper-K | 中微子质量排序、CP 相角 |
