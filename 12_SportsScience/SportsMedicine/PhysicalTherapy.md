...
```mermaid
graph TD
    A[初诊评估<br/>Initial Assessment] --> B[临床诊断<br/>Clinical Diagnosis]
    B --> C[制定治疗计划<br/>Treatment Planning]
    C --> D[实施干预<br/>Intervention]
    D --> E[再评估<br/>Re-evaluation]
    E --> F{目标达成?}
    F -->|否| C
    F -->|是| G[出院/转介<br/>Discharge/Referral]
```
