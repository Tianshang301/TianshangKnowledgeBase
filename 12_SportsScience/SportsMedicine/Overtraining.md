...
```mermaid
graph TD
    A[过度训练<br/>Overtraining] --> B[生理症状<br/>Physiological]
    A --> C[心理症状<br/>Psychological]
    A --> D[表现下降<br/>Performance Decline]
    B --> B1[疲劳 Fatigue]
    B --> B2[睡眠障碍 Sleep Disorder]
    B --> B3[静息心率↑ RHR Increase]
    C --> C1[情绪低落 Depression]
    C --> C2[动力下降 Motivation Loss]
    C --> C3[易怒 Irritability]
    D --> D1[力量下降 Strength]
    D --> D2[耐力下降 Endurance]
    D --> D3[协调性下降 Coordination]
```
...
```mermaid
graph LR
    A[准备期<br/>Preparation] --> B[强化期<br/>Intensification]
    B --> C[减量周<br/>Deload Week]
    C --> D[竞赛期<br/>Competition]
    D --> E[过渡期<br/>Transition]
    E --> A
    style C fill:#90EE90
```
