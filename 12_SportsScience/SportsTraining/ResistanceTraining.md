...
```mermaid
graph TD
    A[抗阻训练动作<br/>Resistance Exercises] --> B[上肢 Upper Body]
    A --> C[下肢 Lower Body]
    A --> D[核心 Core]
    B --> B1[推 Push]
    B --> B2[拉 Pull]
    ...
    C --> C2[铰链 Hinge]
    C --> C3[单腿 Single Leg]
    D --> D1[抗伸展 Anti-Extension]
    D --> D2[抗旋转 Anti-Rotation]
    D --> D3[抗侧屈 Anti-Lateral Flexion]
```
...
```mermaid
graph LR
    A[准备期<br/>Preparatory] --> B[第一过渡<br/>First Transition]
    B --> C[竞赛期<br/>Competition]
    C --> D[第二过渡/减量<br/>Taper]
    D --> E[过渡期<br/>Transition/Off-Season]
    E --> A
```
