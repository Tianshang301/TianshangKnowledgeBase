...
```mermaid
flowchart LR
    subgraph 训练原则 Principles
        A[多平面运动<br/>Multi-planar] 
        B[核心优先<br/>Core First]
        C[地面反作用力<br/>GRF]
        D[本体感觉<br/>Proprioception]
        E[运动链整合<br/>Kinetic Chain]
    end
    A --> F[功能性动作<br/>Functional Movement]
    B --> F
    C --> F
    D --> F
    E --> F
    F --> G[可迁移至专项/生活<br/>Transfer to Sport/ADL]
```
...
```mermaid
flowchart TD
    subgraph 矢状面 Sagittal
        S1[深蹲 Squat]
        S2[弓步 Lunge]
        S3[硬拉 Deadlift]
        S4[推拉 Press/Row]
    end
    subgraph 冠状面 Frontal
        F1[侧弓步 Lateral Lunge]
        F2[单腿侧跳 Lateral Hop]
        F3[侧平板 Side Plank]
    end
    subgraph 水平面 Transverse
        T1[旋转推 Rotational Press]
        T2[伐木 Chop/Lift]
        T3[旋转冲刺 Rotation Sprint]
    end
    S1 & S2 & S3 & S4 --> |下肢力量| M[多平面整合训练<br/>Multi-planar Training]
    F1 & F2 & F3 --> |侧向稳定| M
    T1 & T2 & T3 --> |旋转爆发力| M
```
